...
async function initialize() {
    try {
        ...
        if (!identity) {
            console.log('An identity for the user "admin1" does not exist in the wallet');
...
async function addUser(name, email, type) {
    try {
        ...
        const result = await contract.submitTransaction('addUser', name, email, type);
